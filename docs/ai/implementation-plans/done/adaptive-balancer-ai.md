...
> **Status: implemented. Three parts of this plan were dropped after measuring them.**
> The plan is kept as originally written so the deviations are readable against it. Where the
> shipped code differs, the "Outcome" section at the bottom is authoritative.
...
Everything here was built and then measured with `AiSkirmishHarness` (one balancer against four
experts, 40 seeded medium-map skirmishes, 300 rounds max). The headline metric is the balancer's
share of the board at the end of a match: an even five-way split is 20%, and it is far less noisy
than win counts, which at this batch size are dominated by chance.

| Variant | Board share | Wins/40 | Sealed provinces |
| --- | --- | --- | --- |
| Baseline (before this work) | 29.2% | 11 | 95 |
| Step 1 guards only | 30.0% | 12 | 45 |
| **Plan as written (Steps 1-4)** | **10.0%** | **4** | **62** |
| Modes reordered only, no spending gates | 25.0% | 10 | 31 |
| + threat recalibrated | 27.5% | 11 | 36 |
| **Shipped** | **30.0-32.5%** | **12-13** | **23-32** |

The harness is not bit-reproducible across JVM runs (identical code scored 30.0% and 32.5% on
consecutive runs), so treat differences under about 3 points as noise.

### What shipped as planned

- **Step 1 in full.** The seal guards, the scored farm placement and the `propagationList`
  initialisation. This is the part that works: sealed provinces fell from 95 to ~30 per 40 matches
  with no strength cost. It benefits every difficulty, since the guards live in the shared base.
- **Steps 2 and 3's structure.** `ProvinceMode`, `BalancerBrain`, the per-turn mode cache, the
  neutral-land attack bias, and mode-driven spending order in both balancer classes.
- **Step 5.** No ship code was added, and neutral counting uses land adjacency only.

### What was dropped, and why

1. **Modes must never switch a category of spending off.** As specified, EXPAND bought units before
   farms and TURTLE hoarded money for a coordinated push. Measured separately, EXPAND-only scored
   10.0% and TURTLE-only 5.0%, against 30.0% for not having modes at all - the two together cost
   about two thirds of the balancer's territory. Income compounds and idle money does not, so a
   province that skips farms or stops attacking never catches up. Modes now only re-order
   priorities (EXPAND defers towers) and re-aim attacks; TURTLE's sole restraint is demanding an
   8-turn income runway instead of 5 before buying.
2. **The `isHexInDanger` tower gate is gone.** Building towers only where a neighbour could
   plausibly take the hex sounds right and measured worse (27.5% with it, 30.0% without): the
   frontline hexes it declined to fortify were the ones that later got taken. This is the weakest
   of the three findings - the gap is near the noise floor - but the gate is also the more complex
   option, so it lost on both counts.
3. **`countNearbyEnemyStrength` sums, and the mode census proved it wrong.** Summing every
   neighbouring province's strength and comparing it to one province's own classified ~85% of
   provinces as TURTLE. It now compares against the strongest *single* neighbour. TURTLE is still
   the most common mode by far (~82%), because the `fortifiedFrontierRatio > 0.5` test is nearly
   always true once armies exist - any enemy hex next to a unit has a non-zero defense number.
   Retuning that test is the obvious next lever if someone picks this up again.

### Unplanned fix found along the way

`checkToKillRedundantUnits` iterated every province on the board, including other players'. It
spent their money and downgraded their units - an AI reaching into opponents' provinces. It is now
filtered to the AI's own fraction. Measured on its own this is strength-neutral.

### Verification

- `make build` compiles.
- `AiSkirmishHarness` passes both of its checks: board share 32.5% against a 25% bar, and 0.8
  sealed provinces per run against a budget of 1.0. The old "win the majority of runs" bar from
  this plan was unreachable by construction in a five-way free-for-all and was replaced.
- Slay rules smoke-tested via `-PappArgs="10,200,true"` (the balancer took 50% of the board there,
  on a 10-run sample too small to tune against).
- The three thresholds this plan asked to tune (`/8` neutral divisor, `3/2` enemy ratio, push
  threshold) partly went away with the push logic; the survivors are documented constants in
  `BalancerBrain`.
